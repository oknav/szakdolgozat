...
class BuildModel_UNet(NeuralNet):
    """
    Build a customized deep UNet with the given filter numbers.
    Arguments:
    ...
    def __init__(self, inp_shape, deepness, filters):
        ...
        self.build_unet(inp_shape, deepness, filters)

    def build_unet(self, inp_shape, deepness, filters):
        ...
        for j, i in enumerate(range(len(up_layers) - 2, -1, -2)):
            ...
            concat = tf.keras.layers.concatenate([down_layers[i], up_layers[i]], axis=3)
            if i in valids:
                up_layers[i - 1] = tf.keras.layers.Conv2DTranspose(filters=filters[deepness - 1 - j],
        ...
        out_layer = tf.keras.layers.Conv2DTranspose(filters=inp_shape[2], kernel_size=(3, 3), padding='same',
                                                    activation='relu')(up_layers[0])
